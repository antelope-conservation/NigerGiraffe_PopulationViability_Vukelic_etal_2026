...
#Doing multistate CMR analysis to see the effect of precipitation on survival of the West African giraffe population in the Giraffe Zone, Niger, 2005 - 2019

To quantify daily precipitation within the giraffe zone (GZ), we used Google Earth Engine (GEE) to extract data from the Climate Hazards Group InfraRed Precipitation with Station (CHIRPS) dataset for our polygon (hereafter “GZ_simple”) representing the giraffe habitat area.
...
Loading libraries and encounter histories data for giraffe
```{r}
...
1. Testing effect of annual precipitation on survival - part of preliminary analysis, and to use precipitation data for JJA analysis
...
# Step 3: Extract year and calculate total annual precipitation
annual_precip <- DP_GZS_05_18 %>%
  mutate(year = year(date)) %>%
  group_by(year) %>%
  summarise(annual_precip = sum(precip_mm, na.rm = TRUE))  

# Step 4: Merge with capture-recapture dataset
...
2. Testing effect of JJA precipitation on survival
